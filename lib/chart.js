var Chart = function(config) {
  ...
  var buildMapping = function() {
    ...
    datasets.forEach(function(dataset, index) {
      if (index > 0) {
        xMin = Math.min(mini(datasets.x), xMin);
        yMin = Math.min(mini(datasets.y), yMin);
        xMax = Math.max(maxi(datasets.x), xMax);
        yMax = Math.max(maxi(datasets.y), yMax);
      }
    });
