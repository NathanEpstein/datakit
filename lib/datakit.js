...
var plot = module.exports.plot = function(){
  ...
  var head = '<html><head><script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/1.0.1/Chart.min.js"></script><style type="text/css">canvas{margin:5em;}</style></head><body><canvas id="myChart" width="700" height="400"></canvas><script type="text/javascript">var ctx = document.getElementById("myChart").getContext("2d");var data = {labels:';
  var tail =']};var myLineChart = new Chart(ctx).Line(data,{responsive:true});</script></body></html>';
  var html = head +'['+ xStr + '], datasets: [' + yStr + tail;
  // fs.writeFile('plot.html',html,function(){
  //   exec('open plot.html');
  // });
